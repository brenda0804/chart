...
- **종목 검색으로 관심종목 추가**: 이름/코드 일부만 입력 → 검색 → 선택 (코드 자동 입력)
- **관심 종목 CRUD**: 사이드바에서 추가/수정/삭제 (`data_store.json` 저장)
- **인터랙티브 차트(Plotly)**: 마우스 오버로 시/고/저/종가 확인, 드래그 확대·축소, **시간 구간 포커스 슬라이더**
  - 일봉: 캔들 + 이평선(5·20) + 거래량 + **외인/기관/개인 누적 수급**
  - 1분봉: 캔들 + 이평선 + 거래량
- **1,000만 원 최적 단타 하이라이트**: 선택 구간에서 수익 최대 매수→매도 구간을 음영 표시 + 매수/매도 시각·가격·수량·수익금 상세 패널. **직접 매수/매도 시점을 골라 손익 시뮬레이션**도 가능
- **분석 데이터 자동 저장**: `outputs/[종목명]/[날짜]_min.csv` → 역추적 시 인터랙티브로 재생성
- **날짜별 메모(매매일지)**: 종목·날짜별 메모를 데이터 경로와 함께 저장
- **종합 매매일지(모아보기 + 역추적)**: 모든 메모를 검색·필터, 행 클릭 시 당시 차트를 **인터랙티브**로 즉시 로드해 복기

> **최적 단타는 사후(hindsight) 복기 지표입니다.** 하루 데이터를 되짚어 계산하며 실시간 매매신호가 아닙니다. '오늘' 분석은 현재 시각까지의 캔들만 사용합니다.
